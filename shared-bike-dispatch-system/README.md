...
1. 登录系统：使用 `alice / password123` 登录，进入运营看板，说明系统包含用户、站点、设备、订单、优惠券、流水和调度模块。
2. 查看基础数据：进入“站点”和“设备”页面，展示站点容量、车辆类型、车辆状态和所属站点。
3. 演示借车：进入“订单”页面，点击“演示下单”，系统会自动选择一个用户和一辆空闲车辆创建进行中订单，并把车辆状态更新为使用中。
4. 演示还车计费：在刚创建的订单上点击“还车计费”，选择归还站点和模拟骑行时长，系统会计算费用、扣减余额、生成交易流水，并把车辆恢复为空闲。
5. 查看交易结果：点击订单行中的“流水”，进入流水页面，展示本次订单支付记录和用户余额变化。
6. 演示维修闭环：进入“调度”页面，在维修记录中点击“开始处理”，再点击“完成维修”，系统会同步更新维修状态和车辆状态。
7. 演示调度工单：在“调度与售后”页面查看演示调度工单，点击“外勤接单”和“妥投归档”，展示调度任务状态流转。
8. 回到看板：刷新“看板”页面，展示订单数、已使用优惠券、调度任务和待处理故障等指标变化。

演示重点可以概括为：

- 业务闭环：借车、还车、计费、流水。
- 运维闭环：报修、处理、恢复车辆。
- 调度闭环：发现站点失衡、生成任务、接单完成、车辆迁移。
- 数据闭环：所有业务操作都会反映到看板和列表数据中。
